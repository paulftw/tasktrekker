...
if (!SUPABASE_URL || !SUPABASE_KEY) {
  throw new Error('Integration tests need NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in .env.local');
}
...
describe('status mutation round-trip via pg_graphql', () => {
  it('persists the updated status', async () => {
    ...
    const { data, error } = await supabase.from('issues').select('status').eq('id', issue.data.id).single();
    if (error) throw error;
...
describe('labels.color check constraint', () => {
  it('rejects non-hex color', async () => {
    const { error } = await supabase.from('labels').insert({ name: `bad-${stamp()}`, color: 'zzz' });

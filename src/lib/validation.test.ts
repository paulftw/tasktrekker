...
describe('issueDescriptionSchema', () => {
  ...
  it('accepts normal prose', () => {
    expect(issueDescriptionSchema.safeParse('Multi\nline body').success).toBe(true);
  });

  it('accepts exactly 10000 characters', () => {
    expect(issueDescriptionSchema.safeParse('a'.repeat(10000)).success).toBe(true);
  });

  it('rejects 10001 characters', () => {
    expect(issueDescriptionSchema.safeParse('a'.repeat(10001)).success).toBe(false);
  });
});

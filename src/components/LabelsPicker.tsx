...
const addMutation = graphql`
  mutation LabelsPickerAddMutation($issue_id: Int!, $label_id: Int!) {
    insertIntoissue_labelsCollection(
      objects: [{ issue_id: $issue_id, label_id: $label_id }]
    ) {
      records {
        nodeId
        issue_id
...
const removeMutation = graphql`
  mutation LabelsPickerRemoveMutation($issue_id: Int!, $label_id: Int!) {
    deleteFromissue_labelsCollection(
      filter: {
        issue_id: { eq: $issue_id }
        label_id: { eq: $label_id }
      }
      atMost: 1
    ) {
      records {
...
function removeEdgeByLabelNumber(
...
export function LabelsPicker({
  ...
  const [remove, removing] = useMutation<LabelsPickerRemoveMutation>(
    removeMutation,
  );
  ...
  function removeLabel(label: Label) {
    ...
    remove({
      ...
      optimisticUpdater: (store) =>
        removeEdgeByLabelNumber(store, issueNodeId, label.number),
      updater: (store) =>
        removeEdgeByLabelNumber(store, issueNodeId, label.number),
      onError: (err) => {
        const msg = err instanceof Error ? err.message : "Unknown error";

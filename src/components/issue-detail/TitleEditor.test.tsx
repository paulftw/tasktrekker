...
function renderEditor(props: { nodeId?: string; number?: number; title?: string }) {
  const environment = createMockEnvironment();
  render(
    <RelayEnvironmentProvider environment={environment}>
      <TitleEditor nodeId={props.nodeId ?? 'issues:1'} number={props.number ?? 7} title={props.title ?? 'Original'} />
...
describe('TitleEditor', () => {
  ...
  it('cancels edit on Escape without committing', async () => {
    ...
    expect(environment.mock.getAllOperations()).toHaveLength(0);
    expect(screen.getByRole('button', { name: /edit title/i })).toHaveTextContent('Original');
  });
});
